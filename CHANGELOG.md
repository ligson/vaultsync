...
- 同步主页新增设备筛选：存在多台设备时默认只展示当前设备目录，可切换到“全部设备”或指定其他设备查看其只读目录；目录数量和待处理问题会跟随筛选范围变化，其他设备目录继续禁止扫描、上传、管理和删除操作。对于服务端仅保存路径哈希、无法安全恢复原名称的其他设备自定义目录，继续显示同步目录短 ID，避免伪造目录名或泄露本地明文路径。本次仅调整客户端展示与筛选状态，不修改服务端数据、SQLite、对象文件、客户端目录绑定、上传队列、同步历史或加密密钥。
