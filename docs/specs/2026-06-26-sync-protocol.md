...
- 游标基于服务端统一事件表 `sync_events` 的单调位置。
...
V1 已支持远端删除墓碑。客户端需要删除远端逻辑文件时调用：

```text
DELETE /api/v1/objects/{object_id}?sync_root_id={sync_root_id}&device_id={device_id}
```

服务端行为：

- 校验当前用户拥有请求中的设备和同步目录。
- 追加 `file_tombstones` 记录。
- 追加 `sync_events` 中的 `delete` 事件。
- 不删除历史密文对象和历史版本记录，便于审计、恢复和后续版本保留策略扩展。

上传后本地清理策略中的“删除本地文件”仅表示客户端本地删除，不表示远端删除。远端删除必须通过独立的 `DELETE` 接口表达，不能复用本地清理策略。
...
- 拉取变更可重复执行，客户端用变更项 `id` 去重；`upsert` 事件的 `id` 是 `version_id`，`delete` 事件的 `id` 是墓碑事件 ID。
