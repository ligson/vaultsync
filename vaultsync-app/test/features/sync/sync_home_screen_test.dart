...
void main() {
  ...
  testWidgets('sync home shows retained files pending after delete policy', (
    ...
    final modifiedAt = DateTime.utc(2026, 8, 4, 9);
    await tester.pumpWidget(
      MaterialApp(
        home: SyncHomeScreen(
          storage: FakeSessionStore(
  ...
  testWidgets('saving existing media backup updates instead of creating root', (
    tester,
  ) async {
    ...
    final sources = FakeMediaBackupSourceStore([
    ...
    expect(sources.saved, hasLength(1));
    expect(sources.saved.single.syncRootId, 'media-root');
  });
  ...
  testWidgets(
    'sync home shows retained clean task pending under delete policy',
    (tester) async {
      final syncRoots = FakeSyncRootGateway([
        const SyncRoot(
          id: 'root-1',
  ...
  testWidgets('sync home downloads a server backup to selected location', (
    ...
    final downloads = FakeRemoteFileDownloadGateway();
    final saver = FakeRemoteFileSaveGateway();
    await tester.pumpWidget(
      MaterialApp(
        home: SyncHomeScreen(
          storage: FakeSessionStore(
          ...
          remoteMetadataDecrypter: const FakeRemoteMetadataDecrypter({
            'object-report': RemoteBackupEntry(
              syncRootId: 'root-1',
              objectId: 'object-report',
              versionId: 'version-report',
              name: 'report.bin',
          ...
          remoteFileDownloads: downloads,
          remoteFileSaver: saver,
    ...
    expect(downloads.token, 'server-token');
    expect(downloads.entry?.encryptedName, 'encrypted-report');
    expect(downloads.entry?.metadataJson, '{"format":"test"}');
    expect(saver.fileName, 'report.bin');
    expect(saver.platform, 'windows');
    expect(saver.writtenBytes, <int>[1, 2, 3, 4]);
    expect(find.text('下载完成：C:\\Downloads\\report.bin'), findsOneWidget);
  });
  ...
  testWidgets('sync home renders large file lists in batches', (tester) async {
    ...
    expect(find.text('file-159.txt'), findsOneWidget);
    expect(find.text('file-179.txt'), findsNothing);
  ...
  testWidgets('collapsed sync root defers file browser work until expanded', (
    tester,
  ) async {
      ...
      MaterialApp(
        home: SyncHomeScreen(
          ...
          syncRootMappings: FakeSyncRootMappingStore([
            const LocalSyncRootMapping(
              syncRootId: 'root-a',
              localPath: '/Users/alice/PhotosA',
              encryptedPath: 'base64:path-a',
              cleanupPolicy: 'keep',
              archivePath: '',
            ),
            const LocalSyncRootMapping(
              syncRootId: 'root-b',
              localPath: '/Users/alice/PhotosB',
              encryptedPath: 'base64:path-b',
              cleanupPolicy: 'keep',
              archivePath: '',
            ),
          ]),
          uploadTasks: FakeUploadTaskStore([
            LocalUploadTask(
              id: 'root-a:a.txt',
              syncRootId: 'root-a',
              localPath: '/Users/alice/PhotosA/a.txt',
              relativePath: 'a.txt',
              sizeBytes: 1,
              modifiedAt: now,
              status: 'pending',
            ...
            LocalUploadTask(
              id: 'root-b:b.txt',
              syncRootId: 'root-b',
              localPath: '/Users/alice/PhotosB/b.txt',
              relativePath: 'b.txt',
              sizeBytes: 1,
              modifiedAt: now,
              status: 'pending',
    ...
    await tester.tap(find.text('PhotosA').first);
    await tester.pump();
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: SyncHomeScreen(
          ...
          uploadTasks: FakeUploadTaskStore(),
          syncRoots: roots,
          folderPicker: const FakeFolderPicker(
  ...
  testWidgets('scanning an unbound WeChat root explains how to bind it', (
    ...
    await tester.pumpAndSettle();
    await tester.tap(
      find.byKey(const ValueKey('sync_root_quick_actions_wechat-root')),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('sync status paginates large failed task lists', (tester) async {
    final uploadTasks = FakeUploadTaskStore([
      for (var index = 0; index < 81; index += 1)
        LocalUploadTask(
          id: 'root-1:failed-$index.txt',
          syncRootId: 'root-1',
          localPath: '/Users/alice/Photos/failed-$index.txt',
          relativePath: 'failed-$index.txt',
          sizeBytes: 3,
          modifiedAt: DateTime.utc(2026, 6, 27),
          status: 'failed',
          attempts: 1,
    ...
    expect(find.text('失败任务 81 个'), findsOneWidget);
    expect(
      find.byKey(const ValueKey('failed_upload_task_root-1:failed-0.txt')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey('failed_upload_task_root-1:failed-80.txt')),
      findsNothing,
    );
    expect(find.text('第 1 / 3 页'), findsOneWidget);
    expect(
      tester
          .widget<IconButton>(find.byKey(const ValueKey('failed_uploads_next')))
          .onPressed,
      isNotNull,
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final uploadExecutor = RecordingCancelableUploadExecutor();
      ...
      expect(uploadTasks.saved, isEmpty);
      expect(uploadExecutor.pausedSyncRootIds, ['root-1']);
      expect(uploadExecutor.confirmedSyncRootIds, ['root-1']);
      expect(uploadExecutor.resumedSyncRootIds, isEmpty);
    },
  );

  testWidgets('sync home resumes root uploads when deleting root fails', (
    tester,
  ) async {
    final syncRoots = FailingDeleteSyncRootGateway([
      const SyncRoot(
        id: 'root-1',
        userId: 'user-1',
        deviceId: 'device-1',
    ...
    expect(uploadExecutor.pausedSyncRootIds, ['root-1']);
    expect(uploadExecutor.confirmedSyncRootIds, isEmpty);
    expect(uploadExecutor.resumedSyncRootIds, ['root-1']);
    expect(mappings.saved.single.syncRootId, 'root-1');
    expect(uploadTasks.saved.single.syncRootId, 'root-1');
  });
...
class FailingDeleteSyncRootGateway extends FakeSyncRootGateway {
  FailingDeleteSyncRootGateway(super.initialRoots);
...
class FakeWechatFolderDiscovery implements WechatFolderDiscoveryGateway {
  final WechatFolderDiscoveryResult? result;

  const FakeWechatFolderDiscovery(this.result);
...
class RecordingCancelableUploadExecutor
    implements LocalUploadExecutionGateway, LocalUploadCancellationGateway {
  final List<String> pausedSyncRootIds = [];
  final List<String> confirmedSyncRootIds = [];
  final List<String> resumedSyncRootIds = [];

  @override
  void pauseSyncRootUploads(String syncRootId) {
    pausedSyncRootIds.add(syncRootId);
  }

  @override
  void confirmSyncRootDeleted(String syncRootId) {
    confirmedSyncRootIds.add(syncRootId);
  }

  @override
  void resumeSyncRootUploads(String syncRootId) {
    resumedSyncRootIds.add(syncRootId);
  }
  ...
  Future<UploadExecutionResult> executePendingUploads({
    ...
    return const UploadExecutionResult(uploadedCount: 0);
  }
}
...
class FakeRemoteFileDownloadGateway implements RemoteFileDownloadGateway {
  String? token;
  RemoteBackupEntry? entry;
  ...
  Future<RemoteFileDownloadData> load({
    ...
    this.token = token;
    this.entry = entry;
    return RemoteFileDownloadData(name: entry.name, bytes: const [1, 2, 3, 4]);
  }
}

class FakeRemoteFileSaveGateway implements RemoteFileSaveGateway {
  String? fileName;
  String? platform;
  ...
  Future<RemoteFileSaveTarget?> chooseTarget({
    ...
    return const RemoteFileSaveTarget(
      path: r'C:\Downloads\report.bin',
      displayPath: r'C:\Downloads\report.bin',
    );
  }
